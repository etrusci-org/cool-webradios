...
def bake_dist(src_file: pathlib.Path, dist_dir: pathlib.Path) -> None:
    ...
    for station in source:
        for channel in station['channel']:
            m3u_data.append(f"#EXTINF:0,{station['name']} : {channel['name']}\n{channel['url']}")
            html_data.append(f"<li>[<a href=\"{station['url']}\" target=\"_blank\">www</a>] [<a href=\"{channel['url']}\" target=\"_blank\">&gt;&gt;&gt;</a>] <strong>{station['name']}</strong> : {channel['name']}</li>")
